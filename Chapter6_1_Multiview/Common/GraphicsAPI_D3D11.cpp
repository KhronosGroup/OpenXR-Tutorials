...
GraphicsAPI_D3D11::GraphicsAPI_D3D11(XrInstance m_xrInstance, XrSystemId systemId) {
    ...
    D3D11_FEATURE_DATA_D3D11_OPTIONS3 opt3 = {};
    device->CheckFeatureSupport(D3D11_FEATURE_D3D11_OPTIONS3, &opt3, sizeof(D3D11_FEATURE_DATA_D3D11_OPTIONS3));
    if (!opt3.VPAndRTArrayIndexFromAnyShaderFeedingRasterizer) {
        std::cerr << "ERROR: D3D11: Device does not support any shader feeding the rasterizer to set the semantics: SV_ViewportArrayIndex and SV_RenderTargetArrayIndex." << std::endl;
        DEBUG_BREAK;
    }
#endif
}
