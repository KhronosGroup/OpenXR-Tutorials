...
GraphicsAPI_OpenGL::GraphicsAPI_OpenGL(XrInstance m_xrInstance, XrSystemId systemId) {
    ...
    for (GLint i = 0; i < n; i++)
    {
        ...
        foundExtension = strstr((const char*)extension, "GL_OVR_multiview");
        if (foundExtension) {
            break;
        }
    }
    if (foundExtension == nullptr) {
        std::cerr << "ERROR: OPENGL: Unable to find GL_OVR_multiview extension." << std::endl;
        DEBUG_BREAK;
    }
#endif
}
...
size_t GraphicsAPI_OpenGL::AlignSizeForUniformBuffer(size_t size) {
...
void *GraphicsAPI_OpenGL::CreateBuffer(const BufferCreateInfo &bufferCI) {
    ...
    glBufferData(target, (GLsizeiptr)size, bufferCI.data, GL_STATIC_DRAW);
