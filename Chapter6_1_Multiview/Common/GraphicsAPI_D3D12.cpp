...
GraphicsAPI_D3D12::GraphicsAPI_D3D12() {
    ...
    if (opt3.ViewInstancingTier == D3D12_VIEW_INSTANCING_TIER_NOT_SUPPORTED) {
        std::cerr << "ERROR: D3D12: Device does not support D3D12_VIEW_INSTANCING." << std::endl;
        DEBUG_BREAK;
    }
#endif
...
void *GraphicsAPI_D3D12::CreateBuffer(const BufferCreateInfo &bufferCI) {
    ...
    if (bufferCI.type == BufferCreateInfo::Type::UNIFORM) {
        size = AlignSizeForUniformBuffer(size);
    }
...
void *GraphicsAPI_D3D12::CreatePipeline(const PipelineCreateInfo &pipelineCI) {
    ...
    if (pipelineCI.viewMask != 0) {
        ...
        {
            viewInstanceLocations[i].RenderTargetArrayIndex = static_cast<UINT>(i);
        ...
        gpss1.ViewInstancingDesc = CD3DX12_VIEW_INSTANCING_DESC(viewInstancingDesc);

        D3D12_PIPELINE_STATE_STREAM_DESC gpssd;
        gpssd.SizeInBytes = sizeof(gpss1);
        gpssd.pPipelineStateSubobjectStream = &gpss1;

        ID3D12Device2* device2 = nullptr;
        if (device->QueryInterface<ID3D12Device2>(&device2) != S_OK) {
            std::cout << "Error in querying for ID3D12Device2." << std::endl;
            DEBUG_BREAK;
        }

        D3D12_CHECK(device2->CreatePipelineState(&gpssd, IID_PPV_ARGS(&pipeline)), "Failed to create Pipeline.");
    } else {
    ...
    for (const DescriptorInfo &descriptorInfo : descriptorInfos) {
        ...
        case DescriptorInfo::Type::BUFFER: {
            ...
            if (descriptorInfo.readWrite) {
                ...
                cbvDesc.SizeInBytes = AlignSizeForUniformBuffer(descriptorInfo.bufferSize);
